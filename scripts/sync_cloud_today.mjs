...
import { resolveSyncDate } from "../lib/sync-cloud.js";
...
async function getLocalSyncDate() {
...
async function syncArticles() {
  ...
  if (!syncDate) {
    return { count: 0, syncDate: null };
  }

  const result = await localDb.execute({
    sql: `SELECT title, title_zh, url, source, date, published_at, tags, zero_eval, summary, recommend_level, obi_ref, analysis_json, pushed_at, pushed_morning_at, pushed_evening_at, created_at, read_at
      FROM articles
      WHERE date = ?
  ...
  // Ensure remote has analysis_json column (additive migration)
  const remotePragma = await remoteDb.execute("PRAGMA table_info(articles)");
  const remoteHasAnalysis = remotePragma.rows.some((r) => String(r.name) === "analysis_json");
  if (!remoteHasAnalysis) {
    await remoteDb.execute("ALTER TABLE articles ADD COLUMN analysis_json TEXT");
  }
  ...
  for (const row of result.rows) {
    ...
    await remoteDb.execute({
      sql: `INSERT INTO articles (
        ...
        analysis_json, pushed_at, pushed_morning_at, pushed_evening_at, created_at, read_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [
        row.title,
        row.title_zh ?? null,
  ...
  return { count: result.rows.length, syncDate };
}

async function syncObiCard(syncDate) {
  if (!syncDate) {
    return 0;
  }
...
async function syncLesson(syncDate) {
  if (!syncDate) {
    return 0;
  }
...
const { count: articleCount, syncDate } = await syncArticles();
const [obiCount, lessonCount] = await Promise.all([syncObiCard(syncDate), syncLesson(syncDate)]);
console.log(`sync_date=${syncDate ?? "none"} synced_articles=${articleCount} synced_obi=${obiCount} synced_lessons=${lessonCount}`);
